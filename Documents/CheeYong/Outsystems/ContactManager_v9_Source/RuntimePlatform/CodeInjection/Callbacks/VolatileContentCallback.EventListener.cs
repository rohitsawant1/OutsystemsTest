...
namespace OutSystems.HubEdition.RuntimePlatform.Callbacks {
    partial class VolatileContentCallback {
        public new class EventListener : AbstractCallback.EventListener {
            public EventListener(VolatileContentCallback callback, CallbackEvent evt, int espaceId, int tenantId) : base(callback, evt, espaceId, tenantId) {
            }

            public override bool NeedsInvoke(AppInfo app, SessionInfo session, CallbackEvent triggeredEvt) {
                return base.NeedsInvoke(app, session, triggeredEvt) && triggeredEvt == CallbackEvent.PageRender;
            }
        }
    }
}