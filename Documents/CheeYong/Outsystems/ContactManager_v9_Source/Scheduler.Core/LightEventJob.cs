...
namespace OutSystems.Scheduler.Core {
    [DoNotObfuscateType]
    [DoNotSealType]
    public class LightEventJob : AbstractEventJob {
        ...
        public bool EventExpired {
            get { return DaysSinceQueued >= context.SettingsProvider.Get(SchedulerSettings.LightEvents.LightProcessBackoffMaxDays); }
        }

        public LightEventJob(string eSpaceKey, long id, int EspaceId, int TenantId, int ActivityId, int ProcessId, int ProcessDefId, string DataId, int DaysSinceQueued, string EspaceName, string TenantName, bool ValidTenant, ObjectKey SSKey, string sandboxKey, RunningContext context, Func<int, bool> sleep)
            : base(id.GetHashCode(), sandboxKey, context, sleep) {
        ...
        public override void Execute() {
            ...
            if (Valid) {
                ...
                using (ActivityHandler activityHandler = new ActivityHandler(context.SettingsProvider, Url(), TenantId, 0, SchedulerUtils.SchedulerConsumerKey, this.eSpaceKey)) {                   
                    ...
                    if (ProcessDefId != 0) {
                        OSTrace.Debug("EVENT SCHEDULER DEBUG START: Going to execute light event WS (SSKey=" + keyDatabaseValue + ",DataId=" + DataId + "), Thread: " + Thread.CurrentThread.ManagedThreadId);
                        ...
                        OSTrace.Debug("ACTIVITY SCHEDULER DEBUG START: Going to execute light activity WS (SSKey=" + keyDatabaseValue + ",ActivityId=" + ActivityId + ",ProcessId=" + ProcessId + ",DataId=" + DataId + "), Thread: " + Thread.CurrentThread.ManagedThreadId);
                ...
                OSTrace.Debug(String.Format("EVENT SCHEDULER DEBUG: Successfully executed light event (id={0},eSpace={1}) for {2} {3}, Thread: {4}",
                ...
                ErrorLog.LogApplicationError(EspaceId, TenantId, String.Format(@"Scheduler Service: Discarding light event for {0} {1} because it is invalid.", (ActivityId != 0 ? "Activity" : "Process"), (ActivityId != 0 ? ActivityId : ProcessDefId)), string.Empty, null, "Scheduler", eSpaceName, applicationName, applicationKey);
                OSTrace.Debug("EVENT SCHEDULER DEBUG: Light event execution discarded because the event is invalid (SSKey=" + keyDatabaseValue + ",ActivityId=" + ActivityId + ",EspaceId=" + EspaceId+ ",EspaceName=" + EspaceName + ",ProcessDefId = " + ProcessDefId+ ", ProcessId=" + ProcessId + ",ValidTenant=" + ValidTenant + ",DataId = " + DataId + "), Thread: " + Thread.CurrentThread.ManagedThreadId);
            }
        ...
        public override void HandleError(DateTime startDatetime, Exception ex, int durationInSec) {
            ...
            try {
                if (isAlreadyBeingProcessed) {
                    ...
                    if (EventExpired) {
                        ErrorLog.LogApplicationError(EspaceId, TenantId, String.Format(@"Scheduler Service: Discarding light event for {0} {1} because it exceeded the maximum time for retries({2} days).", (ActivityId != 0 ? "Activity" : "Process"), (ActivityId != 0 ? ActivityId : ProcessDefId), context.SettingsProvider.Get(SchedulerSettings.LightEvents.LightProcessBackoffMaxDays)), string.Empty, null, "Scheduler", eSpaceName, applicationName, applicationKey);
                        RemoveEvent();
                    } else {
                        RequeueEvent(true);                       
                    }
                ...
                SchedulerUtils.LogError(String.Format("Scheduler Service: Error handling error of light event (id={0})", Id.ToString()), e, SandboxKey, logToEventViewer: true);
            }
        }
        ...
        protected void RequeueEvent(bool hasError) {
            ...
            RetryOnError(
                "updating light event (id= {0})".F(Id.ToString()),
                () => {
                    using (Transaction privTrans = context.DatabaseProvider.GetCommitableTransaction()) {
                        DBRuntimePlatform.Instance.RequeueLightEvent(privTrans, Id, hasError);
                        privTrans.Commit();
                    }
                    OSTrace.Debug(String.Format("EVENT SCHEDULER DEBUG: Successfully updated (and requeued) light event (id={0},eSpace={1}). " +
                            (hasError ? "An error has occurred during the previous execution of the light event, " : "An attempt has occurred to run several events at the same time, ") + "Thread: {2}",
        ...
        protected void RemoveEvent() {
            RetryOnError(
                "deleting light event (id= {0})".F(Id.ToString()),
                () => {
                    using (Transaction privTrans = context.DatabaseProvider.GetCommitableTransaction()) {
                        DBRuntimePlatform.Instance.DeleteLightEvent(privTrans, Id);
                        privTrans.Commit();
                    }

                    OSTrace.Debug(String.Format("EVENT SCHEDULER DEBUG: Successfully deleted light event (id={0},eSpaceId={1}), Thread: {2}",
                        Id.ToString(), EspaceId.ToString(), Thread.CurrentThread.ManagedThreadId));
                });
        }
